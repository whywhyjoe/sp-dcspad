...
node monaco.mjs     # 22 checks: models, diagnostics, PnPjs + Alpine JS/HTML intelligence, legacy catalogs, isolated snippet undo/redo, assets and worker degradation
node config.mjs     #  6 checks: config URL resolution, intelligence metadata, and ordered framework fallback
node hosted.mjs     # 10 checks: early/delayed splash, exact boot/bundle/config path, hosted flag/base, versioned runtime/types and same-origin worker
node darkmode.mjs   #  8 checks: preview theme toggle + user-CSS-wins layering
node splash.mjs     #  3 checks: boot splash lifecycle
```

`npm test` runs all 98 checks in that order. Exit code is non-zero on any
