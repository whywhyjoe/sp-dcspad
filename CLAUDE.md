...
5. **No framework, no user build — but the hosted artifact is bundled.**
   ...
   configured BMO design-system CSS or the Fluent icon catalog; re-run
   `build:vendor` after bumping Turndown.** Monaco lives under `vendor/monaco/` as one
...
vendor/turndown/          generated Turndown browser-ESM copy (HTML→Markdown engine)
tools/build-monaco.mjs    reproducible Monaco/PnPjs vendor build; never hand-edit its output
tools/build-vendor-turndown.mjs vendors turndown's browser ESM build; the relative
                          import is what makes src/ work bundled AND unbundled
...
src/html-markdown.js      the one HTML→Markdown converter (Turndown + SharePoint
                          rules) behind two profiles: 'listField' (headings→bold,
                          pipe rows) for list field values, 'pageContent' (demoted
                          ATX headings, real tables) for page text web parts
...
src/workbench/            SP Workbench, a site inspector (read + Tier 2 edit tools):
                          ...
                          folder AddUsingPath, mock writer recording to
                          ...
                          (default; sanitized then converted, falling back to
                          HTML only where markdown cannot carry the part) and
                          'html' (the sanitized source markup, for a page the
                          conversion got wrong); both are .md and differ only
                          in the content blocks, contentFileName() keeping the
                          stems apart; location line, merged content,
                          ...
                          rich text via src/html-markdown.js, attachment links,
                          ...
                          grid.js sort/filter/export/copy-as, row selection (selection-scoped exports), column resizers + render cells (columns marked `action` host per-row controls and are dropped from every export) ·
