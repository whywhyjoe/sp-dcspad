...
src/workbench/            SP Workbench, a site inspector (read + Tier 2 edit tools):
                          ...
                          and /sites/target list-schema webs + a stateful
                          mockWriter) ·
                          grid.js sort/filter/export/copy-as, row selection (selection-scoped exports), column resizers + render cells (columns marked `action` host per-row controls and are dropped by export.js from CSV/JSON/markdown), createMenuButton (Export ▾ style menus) ·
