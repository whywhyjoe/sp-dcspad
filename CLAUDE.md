...
- **Site Inspector** — v1 SHIPPED as the **SP Workbench** second entry point
  ...
  shipped stage 1a (capture/export/import/create/copy). Reserved seams: stage
  1b item data on the same engine (Items fieldset currently disabled), stage
  2 document libraries (schema only; `Copy to…` currently disabled on them).
